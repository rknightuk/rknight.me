...
export const postsForOg = (collectionApi) => {
    return collectionApi.getAll()
        .filter(p => {
        if (['post', 'link'].includes(p.data.postType))
            return true;
...
export const postStats = (collectionApi) => {
    ...
    for (let post of posts) {
        ...
        if (thisYear != currentYear) {
            ...
            statsObject.years.push(makeYearStats(currentYear, yearPostCount, yearWordCount, yearCodeBlockCount, avgDays, yearCharacterCount, yearParagraphCount));
