...
import mastodonCount from './mastodonCounter.js'
import TurndownService from 'turndown'
import utils from './filters/utils.js'

const permalinkSymbol = '∞'

export const tootText = (post) => {
	let content = ''
	...
	if (post.postType === 'almanac')
		return formatAlmanac(post, turnedDown, permalink)
...
const formatAlmanac = (post, content, permalink) => {
	...
	const contentWithReview =
		content.length > 0 ? `${combined}\n\n${content}` : combined
...
const formatLink = (post, content, permalink) => {
	...
	const formattedWithAll = content
		? `${formatted}\n\n${content}\n\n${permalinkSymbol} ${permalink}`
		: `${formatted}\n\n${permalinkSymbol} ${permalink}`
	if (mastodonCount.getMastodonLength(formattedWithAll).length <= 500) {
		return formattedWithAll
	} else if (
		mastodonCount.getMastodonLength(formattedWithFirstLine).length <= 500
	) {
