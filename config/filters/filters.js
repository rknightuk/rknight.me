...
const sortDiscussion = (data) => {
    return Object.values(Object.keys(data).sort().reduce((obj, key) => {
        obj[key] = data[key];
        return obj;
    }, {}));
};
...
const getAlmanacImage = ({ mediaType, tmdbid, giantbombid, customImage, isbn13 }) => {
    let filePath = null;
    if (customImage) {
...
const getBackdropImage = ({ mediaType, tmdbid, customBackdrop, imageIndex }) => {
    let filePath = null;
